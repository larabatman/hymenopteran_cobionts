...
#SBATCH --mem=256G
#SBATCH --job-name=metaasm_metaspades_no_hic
#SBATCH --output=logs/metaasm_metaspades_no_hic_%j.out
#SBATCH --error=logs/metaasm_metaspades_no_hic_%j.err
...
# Moreover, this is the variant without hic parameters enabled
# Usage: 
...
SPECIES="${1}"
RUN_TAG="attempt_illumina_no_hic"
...
# Input files: Illumina cleaned reads
...
METASPADES_FASTA="${PROJECT_ROOT}/assemblies/${SPECIES}/metaspades/${SAMPLE_ID}_metaspades.fasta.gz"
