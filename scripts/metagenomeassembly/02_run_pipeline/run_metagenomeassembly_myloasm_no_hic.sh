...
#SBATCH --mem=256G
#SBATCH --job-name=metaasm_myloasm_no_hic
#SBATCH --output=logs/metaasm_myloasm_no_hic_%j.out
#SBATCH --error=logs/metaasm_myloasm_no_hic_%j.err
...
# Moreover, this is the variant without hic data so hic parameters are disabled 
# Usage:
...
case "${READ_TYPE}" in
    hifi)
        ...
        MYLOASM_FASTA="${PROJECT_ROOT}/assemblies/${SPECIES}/myloasm/${SAMPLE_ID}_myloasm.fasta.gz"
        MIN_PERC_ID=99
        ;;
    ont)
        READS="${PROJECT_ROOT}/reads/processed/${SPECIES}/ont_filtered.fasta.gz"
        MYLOASM_FASTA="${PROJECT_ROOT}/assemblies/${SPECIES}/myloasm_ont/${SAMPLE_ID}_myloasm_ont.fasta.gz"
        MIN_PERC_ID=90
        ;;
esac

PIPELINE_DIR="${PROJECT_ROOT}/metagenomeassembly"
RUN_TAG="attempt_no_hic_myloasm_${READ_TYPE}"
RUN_DIR="${PROJECT_ROOT}/runs/${SPECIES}/${RUN_TAG}"
OUTDIR="${PROJECT_ROOT}/results/${SPECIES}/myloasm_${READ_TYPE}_no_hic_binners"
