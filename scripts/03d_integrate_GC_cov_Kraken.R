...
library(readr) # TSV reading and writing
...
base_dir <- "results"
win_dir <- file.path(base_dir, paste0(SPECIES, "_kraken_windows"))
gc_dir <- file.path(base_dir, paste0(SPECIES, "_gc_cov"))
out_dir <- file.path(base_dir, paste0(SPECIES, "_qc_summary"))

dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

window_file <- file.path(win_dir, "window_taxid_status.tsv")
gc_cov_file <- file.path(gc_dir,  "gc_cov.tsv")

out_prefix  <- file.path(out_dir, "generic")
...
normalize_contig <- function(x){
  x %>%
    ...
    str_remove("_sliding$") %>% # remove trailing "_sliding" if present
...
contig_tsv <- paste0(out_prefix, "_contig_tax_gc_cov.tsv")
write_tsv(contigs, contig_tsv)
# Basepair-weighted summary by contig class
bp_tsv     <- (paste0(out_prefix, "_bp_summary.tsv")
...
plot_png   <- paste0(out_prefix, "_gc_cov_bloblike.png")
